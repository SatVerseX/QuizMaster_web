...
const MostPopularSection = ({ 
  ...
  const getPopularityBadge = (index) => {
    if (index === 0) return { text: "👑 #1 Bestseller", color: "bg-yellow-500", icon: FaTrophy };
    if (index === 1) return { text: "🥈 #2 Popular", color: "bg-gray-500", icon: FaMedal };
    if (index === 2) return { text: "🥉 #3 Choice", color: "bg-orange-600", icon: FaMedal };
            ...
            return (
              ...
              >
                ...
                <div className="absolute top-3 left-3 z-10">
                  <div className={`${popularityBadge.color} text-white px-3 py-1 rounded-full text-xs font-bold flex items-center gap-1`}>
                ...
                {seriesItem.coverImageUrl ? (
                  <div className="aspect-video relative">
                    ...
                    <div className={`hidden absolute inset-0 items-center justify-center ${
                      ...
                      <div className="text-center">
                        <FaCrown className={`w-10 h-10 mx-auto mb-2 ${isDark ? 'text-gray-500' : 'text-gray-400'}`} />
                  ...
                  <div className={`aspect-video flex items-center justify-center ${
                    ...
                    <div className="text-center text-white">
                      ...
                      <p className="text-sm font-semibold">{seriesItem.title}</p>
                    </div>
                  </div>
                )}
                ...
                <div className="p-5">
                  ...
                  <p className={`text-sm mb-3 line-clamp-2 ${
                    ...
                    {seriesItem.description || 'Most popular test series for comprehensive exam preparation'}
                  </p>
                      ...
                      <div className="flex items-center gap-1">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <FiStar
                            key={star}
                            className={`w-3 h-3 ${
                              star <= (seriesItem.rating || 0) ? 'text-yellow-400 fill-current' : 'text-gray-300'
                            }`}
                          />
                        ))}
                      </div>
                  ...
                  <div className={`mb-4 p-3 rounded-lg ${
                    ...
                    <div className="flex items-center gap-2 mb-1">
                      <FiUsers className={`w-3 h-3 ${isDark ? 'text-blue-400' : 'text-blue-600'}`} />
                      <span className={`text-sm font-semibold ${isDark ? 'text-white' : 'text-gray-900'}`}>
                        Community Choice
                      </span>
                    </div>
                    <p className={`text-xs ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>
                      {seriesItem.totalSubscribers || 0} students have chosen this series for their preparation
                    </p>
                  </div>
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      <span className="flex items-center justify-center gap-2">
                        ...
                        {userIsCreator ? 'Manage' : isSubscribed ? 'Continue' : !seriesItem.isPaid ? 'Start Free' : 'Subscribe'}
                      </span>
                    </button>
                  </div>
                </div>
